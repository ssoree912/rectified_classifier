...
from models.latent_velocity import build_latent_velocity_from_checkpoint, euler_transport


def parse_args():
    parser = argparse.ArgumentParser(description="Precompute delta memmap shards from a trained latent velocity model")
    parser.add_argument("--bundle_root", type=str, required=True)
    parser.add_argument("--output_root", type=str, required=True)
    parser.add_argument("--velocity_ckpt", type=str, required=True)
    parser.add_argument("--aux_key", type=str, default="sr")
    parser.add_argument("--splits", type=str, nargs="+", default=["train", "val", "test"])
    parser.add_argument("--batch_size", type=int, default=512)
    parser.add_argument("--device", type=str, default="cuda:0")
    parser.add_argument("--integration_steps", type=int, default=0)
...
def build_manifest(source_manifest: dict, delta_mode: str, velocity_ckpt: Path, integration_steps: int, save_dtype: str, shards: list):
    return {
        ...
        "velocity_ckpt": str(velocity_ckpt.resolve()),
...
def main():
    ...
    checkpoint = torch.load(args.velocity_ckpt, map_location="cpu")
    model, meta = build_latent_velocity_from_checkpoint(checkpoint)
    model = model.to(device)
    model.eval()
    integration_steps = int(args.integration_steps or meta["integration_steps"])
    save_dtype = np.float16 if args.save_dtype == "float16" else np.float32
    output_root = Path(args.output_root).resolve()
    output_root.mkdir(parents=True, exist_ok=True)

    print(f"[VelocityDelta] bundle_root={Path(args.bundle_root).resolve()}")
    print(f"[VelocityDelta] output_root={output_root}")
    print(f"[VelocityDelta] velocity_ckpt={Path(args.velocity_ckpt).resolve()}")
    print(f"[VelocityDelta] device={device}")
    print(f"[VelocityDelta] delta_mode={args.delta_mode}")
    print(f"[VelocityDelta] integration_steps={integration_steps}")
    ...
    for split in args.splits:
        ...
        print(f"[VelocityDelta] split={split} total_count={manifest['total_count']}")
        ...
        out_manifest = build_manifest(
            ...
            velocity_ckpt=Path(args.velocity_ckpt),
        ...
        print(f"[VelocityDelta] split={split} shards={len(shard_entries)} output={split_out}")
